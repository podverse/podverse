...
describe('resolveListFill', () => {
  it('is loading while the current view is unsettled, even with zero rows', () => {
    expect(
      resolveListFill({ isSettled: false, errorKey: null, rowCount: 0 })
    ).toBe('loading');
  });

  it('is loading while unsettled even when an error key is already set', () => {
    expect(
      resolveListFill({ isSettled: false, errorKey: 'errors.generic', rowCount: 0 })
    ).toBe('loading');
  });
  ...
  it('is error when settled with an error key and no rows', () => {
    expect(
      resolveListFill({ isSettled: true, errorKey: 'errors.generic', rowCount: 0 })
    ).toBe('error');
  });

  it('is ready when settled with rows, even if an earlier error key remains', () => {
    expect(
      resolveListFill({ isSettled: true, errorKey: 'errors.generic', rowCount: 3 })
    ).toBe('ready');
  });
