...
const md5Digest = (inputBytes) => {
  ...
  const ff = (aa, bb, cc, dd, x, s, ac) =>
    md5AddUnsigned(
      md5RotateLeft(md5AddUnsigned(md5AddUnsigned(aa, f(bb, cc, dd)), md5AddUnsigned(x, ac)), s),
      bb
    );
  const gg = (aa, bb, cc, dd, x, s, ac) =>
    md5AddUnsigned(
      md5RotateLeft(md5AddUnsigned(md5AddUnsigned(aa, g(bb, cc, dd)), md5AddUnsigned(x, ac)), s),
      bb
    );
  const hh = (aa, bb, cc, dd, x, s, ac) =>
    md5AddUnsigned(
      md5RotateLeft(md5AddUnsigned(md5AddUnsigned(aa, h(bb, cc, dd)), md5AddUnsigned(x, ac)), s),
      bb
    );
  ...
  for (let offset = 0; offset < input.length; offset += 64) {
    ...
    for (let j = 0; j < 16; j += 1) {
      const k = offset + j * 4;
      x[j] = (input[k] | (input[k + 1] << 8) | (input[k + 2] << 16) | (input[k + 3] << 24)) >>> 0;
    }
