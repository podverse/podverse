...
const feedsBase = `${config.api.prefix}${config.api.version}/feeds`;
...
describe('feeds routes', () => {
  ...
  it('GET / returns feeds and pagination for superuser', async () => {
    const res = await request(app).get(`${feedsBase}`).set(adminAuthHeaders(1));
  ...
  it('GET / returns 403 without feed read permission', async () => {
    const res = await request(app).get(`${feedsBase}`).set(adminAuthHeaders(3));
    expect(res.status).toBe(403);
  });

  it('GET / passes sort and order to listFeedOperationsForTable', async () => {
    await request(app)
      .get(`${feedsBase}`)
  ...
  it('GET / returns 400 for invalid sort key', async () => {
  ...
  it('GET /options returns lifecycle, condition, and takedown reason options for superuser', async () => {
    const res = await request(app).get(`${feedsBase}/options`).set(adminAuthHeaders(1));
    expect(res.status).toBe(200);
  ...
  it('GET /options returns 403 for user without feed read', async () => {
    const res = await request(app).get(`${feedsBase}/options`).set(adminAuthHeaders(3));
    expect(res.status).toBe(403);
  });

  it('GET /lookup?podcast_index_id returns 200 with feed', async () => {
    const res = await request(app)
      .get(`${feedsBase}/lookup`)
  ...
  it('GET /lookup with no param returns 400', async () => {
    const res = await request(app).get(`${feedsBase}/lookup`).set(adminAuthHeaders(1));
    expect(res.status).toBe(400);
  });
  ...
  it('PATCH /:id/policy-state returns 200 and audits', async () => {
    ...
    const res = await request(app)
      .patch(`${feedsBase}/9/policy-state`)
      .set(adminAuthHeaders(1))
      .send({
        active_condition_keys: ['spam_detected'],
  ...
  it('PATCH /:id/policy-state requires documentation for takedown', async () => {
    const res = await request(app)
      .patch(`${feedsBase}/9/policy-state`)
      .set(adminAuthHeaders(1))
      .send({ lifecycle_state_key: 'takedown' });
    expect(res.status).toBe(400);
    expect(res.body.message).toMatch(/takedown/i);
  });

  it('PATCH /:id/policy-state returns 403 without update permission', async () => {
    getSnap.mockResolvedValue({ id: 9, lifecycle_state_key: 'active' });
    const res = await request(app)
      .patch(`${feedsBase}/9/policy-state`)
      .set(adminAuthHeaders(2))
      .send({ lifecycle_state_key: 'pending_archive' });
    expect(res.status).toBe(403);
  });

  it('PATCH /:id/policy-state works for read+update permission', async () => {
    getSnap
  ...
  it('PATCH /:id/policy-state rejects unknown lifecycle_state_key (400)', async () => {
    const res = await request(app)
      .patch(`${feedsBase}/9/policy-state`)
      .set(adminAuthHeaders(1))
      .send({ lifecycle_state_key: 'not_a_valid_lifecycle' });
    expect(res.status).toBe(400);
  });

  it('PATCH /:id/policy-state rejects unknown active_condition_keys entry (400) — parity 07b #12', async () => {
    const res = await request(app)
      .patch(`${feedsBase}/9/policy-state`)
      .set(adminAuthHeaders(1))
      .send({
  ...
  it('PATCH /:id/policy-state returns 400 on disallowed lifecycle transition — parity 07b #11', async () => {
    ...
    const res = await request(app)
      .patch(`${feedsBase}/9/policy-state`)
      .set(adminAuthHeaders(1))
      .send({ lifecycle_state_key: 'active' });
    expect(res.status).toBe(400);
    expect(String(res.body.message)).toMatch(/Disallowed lifecycle transition/);
  });

  it('PATCH /:id/policy-state response.body.feed includes frozen lookup contract keys', async () => {
    getSnap.mockResolvedValue({ id: 9, lifecycle_state_key: 'active' });
    findById.mockResolvedValue({ ...sampleFeed });
    const res = await request(app)
      .patch(`${feedsBase}/9/policy-state`)
      .set(adminAuthHeaders(1))
      .send({ spam_item_limit_override: 9000 });
    expect(res.status).toBe(200);
  ...
  it('PATCH /:id/policy-state passes takedown_transitional to update service', async () => {
    getSnap.mockResolvedValue({ id: 9, lifecycle_state_key: 'active' });
    findById.mockResolvedValue({ ...sampleFeed });
