...
export const AddByRSSTracksPageClient: React.FC = () => {
  ...
  const headerTitle = `${tFeatures('add_by_rss.label')} · ${tMedia('music.tracks')}`;
        ...
        <MainInnerWrapper>
          <MainInnerContentWrapper>
            {totalPages > 1 ? (
              ...
              >
                {items.length > 0 ? (
                  <AddByRSSTrackNodes items={items} viewSelected={viewSelected} sortOrder={sort} />
                ) : null}
              </Pagination>
            ) : items.length > 0 ? (
              <AddByRSSTrackNodes items={items} viewSelected={viewSelected} sortOrder={sort} />
