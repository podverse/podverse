...
test.describe('Embed responsive player', () => {
  test('When a single responsive embed is paused, the responsive stage and overlay controls are visible.', async ({
    page,
  }, testInfo) => {
    await page.goto(
      `/embed/episode/${E2E_EMBED_VIDEO_ITEM_ID_TEXT}?presentation=video&player=responsive`
    );
    ...
    await expect(page.getByTestId('embed-responsive-info-overlay')).toBeVisible();
    await expect(page.getByTestId('embed-responsive-controls-overlay')).toBeVisible();
    await expect(page.getByTestId('embed-responsive-mute-toggle')).toBeVisible();

    await capturePageLoad(
      page,
      testInfo,
      'The single responsive embed renders the inline video stage and both overlays.',
      page.getByTestId('embed-responsive-stage')
    );
  });

  test('When a music track embed uses presentation=video without an explicit player param, the responsive player shell renders.', async ({
    page,
  }) => {
    ...
    await expect(page.getByTestId('embed-responsive-controls-overlay')).toBeVisible();
    await expect(
      page.getByTestId('embed-responsive-info-overlay').getByTestId('embed-player-info')
    ).toBeVisible();
  });

  test('When an audio episode uses responsive player with prefer video, the inline video element renders for the video enclosure.', async ({
    page,
  }) => {
    await page.goto(
      `/embed/episode/${EMBED_FIXTURE_PODCAST_EPISODE_AUDIO_ID_TEXT}?presentation=video&player=responsive`
    );
    await expectEmbedResponsiveVideoElement(page);
    await expect(page.getByTestId('embed-responsive-center-art')).toHaveCount(0);
  });

  test('When an audio episode uses responsive player with prefer audio, center artwork renders instead of a video element.', async ({
    page,
  }) => {
    await page.goto(
      `/embed/episode/${EMBED_FIXTURE_PODCAST_EPISODE_AUDIO_ID_TEXT}?presentation=audio&player=responsive`
    );
    await expectEmbedResponsiveCenterArt(page);
    await expect(page.getByTestId('embed-responsive-video-element')).toHaveCount(0);
  });

  test('When a responsive audio episode switches to a video alternate enclosure, the inline video element appears.', async ({
    page,
  }) => {
    await page.goto(
      `/embed/episode/${EMBED_FIXTURE_PODCAST_EPISODE_AUDIO_ID_TEXT}?presentation=audio&player=responsive`
  ...
  test('When a compact player prefers video on a video episode, playback uses the compact shell without a video element.', async ({
    page,
  }) => {
    await page.goto(
      `/embed/episode/${E2E_EMBED_VIDEO_ITEM_ID_TEXT}?presentation=video&player=compact`
    );
    await expectEmbedCompactShellNoVideoElement(page);
    await expect(page.getByTestId('embed-player-controls')).toBeVisible();
  });

  test('When an album list embed uses responsive player with prefer video, the inline video element renders for the active track.', async ({
    page,
  }) => {
    await page.goto(
      `/embed/album/${EMBED_FIXTURE_MUSIC_ALBUM_ID_TEXT}?presentation=video&player=responsive`
    );
  ...
  test('When switching rows in a responsive list embed, the mute toggle stays unmuted.', async ({
    page,
  }) => {
    await page.goto(
      `/embed/podcast/${E2E_EMBED_VIDEO_CHANNEL_ID_TEXT}?presentation=video&player=responsive`
    );
    await expectEmbedListShell(page);
  ...
  test('When hovering the responsive mute control, a popover with a vertical volume bar appears.', async ({
    ...
    const muteToggle = page.getByTestId('embed-responsive-mute-toggle');
    await expect(muteToggle).toBeVisible();
    await expect(page.getByTestId('embed-responsive-volume-popover')).toHaveCount(0);
  ...
  test('When comparing chapter hover behavior, responsive presentation shows a chapter tooltip while compact presentation does not.', async ({
    ...
    await page.goto(`/embed/episode/${EMBED_FIXTURE_PODCAST_EPISODE_AUDIO_ID_TEXT}?player=compact`);
    await expect(page.getByTestId('embed-single-shell')).toBeVisible();
    await hoverProgressBarCenter(page);
    await expect(page.getByRole('tooltip')).toHaveCount(0);

    await page.goto(
      `/embed/episode/${EMBED_FIXTURE_PODCAST_EPISODE_AUDIO_ID_TEXT}?presentation=video&player=responsive&t=${EMBED_SAMPLE_CHAPTER_TWO_START_SECONDS + 6}`
    );
    await expect(page.getByTestId('embed-responsive-stage')).toBeVisible();
  ...
  test('When a responsive list embed uses rows=3, the list region keeps a fixed 3-row viewport.', async ({
    page,
  }) => {
    await page.goto(
      `/embed/podcast/${EMBED_FIXTURE_PODCAST_CHANNEL_ID_TEXT}?presentation=video&player=responsive&rows=3`
    );
