...
/**
 * Validates the web app process env before build. The app only needs RUNTIME_CONFIG_URL;
 * all other config is loaded from the runtime-config sidecar at runtime.
 * The sidecar has its own validation (apps/web/sidecar/src/server.ts) for the full variable list.
