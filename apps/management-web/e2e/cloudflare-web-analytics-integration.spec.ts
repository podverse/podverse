...
(cloudflareEnabledE2e ? test.describe.skip : test.describe)(
  ...
  () => {
    test('When Cloudflare Web Analytics is disabled, the login page does not load the beacon script', async ({
      page,
    }) => {
      await page.goto('/');
...
(cloudflareEnabledE2e ? test.describe : test.describe.skip)(
  ...
  () => {
    test('When Cloudflare Web Analytics is enabled, the login page loads the beacon script with the configured token', async ({
      page,
    }) => {
      await page.goto('/');
