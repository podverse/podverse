...
async function translateObject(
  ...
  for (let i = 0; i < stringsToTranslate.length; i += BATCH_SIZE) {
    ...
    console.log(
      `[${targetLang}] Translating batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(stringsToTranslate.length / BATCH_SIZE)} (${batch.length} strings)...`
    );
  ...
  function rebuildWithTranslations(o: any, stack: string[], existing: any): any {
    ...
    if (Array.isArray(o)) {
      return o.map((item, i) =>
        rebuildWithTranslations(item, [...stack, String(i)], existing?.[i])
      );
    }
