...
export class StatsAggregatedGenerator {
  ...
  private generateBaseStats(): Omit<BaseStatsAggregated, 'id'> {
    ...
    return {
      ...
      week_current_count: Math.floor(
        baseDailyCount * 7 * faker.number.float({ min: 0.8, max: 1.2 })
      ),
      ...
      month_current_count: Math.floor(
        baseDailyCount * 30 * faker.number.float({ min: 0.8, max: 1.2 })
      ),
