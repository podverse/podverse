...
class VideoGenerator {
  ...
  private generateMinimalMp4(id: string, durationSeconds: number): Buffer {
    ...
    const ftyp = this.createBox(
      ...
      Buffer.from([
        ...
        0x73,
        0x6f,
        0x32, // Compatible: iso2
        0x6d,
        0x70,
        0x34,
        0x31, // Compatible: mp41
      ])
    );
    boxes.push(ftyp);
  ...
  private createMvhdBox(durationSeconds: number): Buffer {
    ...
    data.writeUInt32BE(0, offset);
    offset += 4; // Creation time
    data.writeUInt32BE(0, offset);
    offset += 4; // Modification time
    data.writeUInt32BE(1000, offset);
    offset += 4; // Timescale
    data.writeUInt32BE(duration, offset);
    offset += 4; // Duration

    data.writeUInt32BE(0x00010000, offset);
    offset += 4; // Rate (1.0)
    data.writeUInt16BE(0x0100, offset);
    offset += 2; // Volume (1.0)
    offset += 10; // Reserved
  ...
  private createTkhdBox(durationSeconds: number): Buffer {
    ...
    data.writeUInt32BE(640 << 16, offset);
    offset += 4; // Width
  ...
  private createMdhdBox(durationSeconds: number): Buffer {
    ...
    data.writeUInt32BE(1000, offset);
    offset += 4; // Timescale
    data.writeUInt32BE(durationSeconds * 1000, offset);
    offset += 4;
  ...
  private createHdlrBox(): Buffer {
    ...
    data.write('vide', offset, 'ascii');
    offset += 4; // Handler type
    offset += 12; // Reserved
  ...
  private createStblBox(): Buffer {
    ...
    const stsd = this.createBox(
      ...
      Buffer.from([
        ...
        0x00,
        0x00,
        0x00, // Entry count (0 for minimal)
      ])
    );

    const stts = this.createBox(
      'stts',
      Buffer.from([
        0x00,
        0x00,
  ...
  private generateMinimalWebm(id: string, durationSeconds: number): Buffer {
    ...
    // Segment
    const segmentInfo = this.ebmlElement(
      0x1549a966,
      Buffer.concat([
    ...
    elements.push(this.ebmlElement(0x18538067, Buffer.concat([segmentInfo, tracks])));

    return Buffer.concat(elements);
  }

  private createWebmTracks(): Buffer {
    const videoTrack = this.ebmlElement(
      0xae,
      Buffer.concat([
