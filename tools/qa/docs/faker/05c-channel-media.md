...
export class ChannelCategoryGenerator {
  ...
  generate(channel: GeneratedChannel, count: number = 2): GeneratedChannelCategory[] {
    ...
    const allCategories = Object.values(CategoryEnum).filter(
      (v) => typeof v === 'number'
    ) as number[];
    const selectedCategories = faker.helpers.arrayElements(
      allCategories,
      Math.min(count, allCategories.length)
    );
  ...
  generate(channel: GeneratedChannel, count: number = 2): GeneratedChannelImage[] {
    ...
    for (let i = 0; i < count; i++) {
      ...
      images.push({
        ...
        image_width_size: hasWidthSize ? faker.helpers.arrayElement(this.imageSizes) : null,
      });
    }
