...
export class ChannelGenerator {
  ...
  generate(feed: GeneratedFeed): GeneratedChannel {
    const title =
      faker.company.name() +
      ' ' +
      faker.helpers.arrayElement(['Podcast', 'Show', 'Radio', 'Talk', 'Weekly', 'Daily', 'Cast']);
    const sortableTitle =
      createSortableTitle(title)?.slice(0, DATABASE_CONSTANTS.varchar_short) || null;
  ...
  toSQL(channel: GeneratedChannel): string {
    ...
    const sortableTitle = channel.sortable_title
      ? `'${this.escapeSQL(channel.sortable_title)}'`
      : 'NULL';
...
export class ChannelAboutGenerator {
  ...
  generate(channel: GeneratedChannel): GeneratedChannelAbout {
    return {
      ...
      language: faker.helpers
        .arrayElement(this.languages)
