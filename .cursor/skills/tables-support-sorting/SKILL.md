...
- Use **table-sort-defaults** (`.cursor/skills/table-sort-defaults/SKILL.md`) for default **`asc`/`desc`** per column type.
- Use **sort-prefs-cookie-by-path** (`.cursor/skills/sort-prefs-cookie-by-path/SKILL.md`) when list metadata is persisted via cookies and URL stripping (see hooks in **`@podverse/ui`**).
