...
- **Runners and validation:** From repo root, `bash scripts/database/run-linear-migrations.sh --database app|management` (always pass `--database`; wrappers delegate to `infra/k8s/base/ops/source/database/runner/`); `bash scripts/database/validate-linear-migrations.sh` checks filenames, ordering, and that every SQL file under `linear-migrations/app` and `linear-migrations/management` is listed in `infra/k8s/base/ops/kustomization.yaml` (ops bundle must stay in sync with disk).
