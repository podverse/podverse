...
- **Bootstrap in init:** `infra/k8s/base/db/source/bootstrap/` — `0001` and `0002` shell scripts create users/roles and the management database; `0003_apply_linear_baselines.sh` loads generated `0003a_app_linear_baseline.sql.gz` and `0003b_management_linear_baseline.sql.gz` as each DB owner; the generated `0004_seed_linear_migration_history.sql` inserts `linear_migration_history` rows so ops migration jobs skip already-applied files. See `docs/operations/LINEAR-MIGRATIONS.md` and `make db_regen_linear_baseline`. In cluster, **ops** migration jobs apply the same SQL files from ConfigMaps when new `NNNN_*.sql` land after baseline.
- **Runners and validation:** From repo root, `bash scripts/database/run-linear-migrations.sh --database app|management` (always pass `--database`; wrappers delegate to `infra/k8s/base/ops/source/database/runner/`). In K8s, migration jobs use **admin** DB credentials (`DB_APP_ADMIN_*` / `DB_MANAGEMENT_ADMIN_*`), not read-write users; use `LINEAR_MIGRATIONS_BASE_DIR` (or `LINEAR_MIGRATIONS_DIR`) for mounted SQL paths. `bash scripts/database/validate-linear-migrations.sh` checks filenames, ordering, and that every SQL file under `linear-migrations/app` and `linear-migrations/management` is listed in `infra/k8s/base/ops/kustomization.yaml` (ops bundle must stay in sync with disk).
