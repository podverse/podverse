...
- **Bootstrap in init:** `infra/k8s/base/db/source/bootstrap/` — `0001` and `0002` create roles/databases (including `DB_*_OWNER_*` and `DB_*_MIGRATOR_*` roles), `0003_apply_linear_baselines.sh` installs extensions as owner roles then loads generated `0004_app_linear_baseline.sql.gz` and `0005_management_linear_baseline.sql.gz` as migrator roles; these generated baselines include deterministic `linear_migration_history` rows so ops jobs skip already-applied files. See `docs/operations/database/LINEAR-MIGRATIONS.md` and `make db_regen_linear_baseline`.
- **First deploy (K8s):** `infra/k8s/base/ops/db-bootstrap-migrations.job.yaml` is an Argo CD PostSync hook that runs app + management migrations once per cluster/DB (marker in `podverse_k8s_bootstrap_state`). Manual suspended migrate CronJobs remain for schema updates and recovery.
