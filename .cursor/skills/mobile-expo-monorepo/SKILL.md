...
| `cannot find 'exsqlite3_open' in scope` (+ log note `could not resolve libclang.dylib … did not match the configured compiler`) | Nix shell `CC`/`CXX`/`LD`/`AR` leak into `xcodebuild`; Xcode 27 then disables explicit modules, so `-import-underlying-module` Swift pods lose their C symbols | Build via `mobile:ios` / `mobile:pod-install` (both unset those vars); never bare `xcodebuild` / `expo run:ios` from a direnv shell |
| `module '_Builtin_stdarg' is needed but has not been provided` / `failed to emit precompiled module '…ExpoSQLite-*.pcm'` | expo-sqlite's public `sqlite3.h` shadows the SDK header, so Xcode 27's explicit-module scan resolves the include to the SDK `SQLite3` module and omits builtin modules the real compile needs | Pods build Swift modules implicitly (`SWIFT_ENABLE_EXPLICIT_MODULES = NO`) via `withPodverseIosPodBuildSettings` + `ensure-ios-pod-build-settings.sh`; app target keeps explicit modules |
