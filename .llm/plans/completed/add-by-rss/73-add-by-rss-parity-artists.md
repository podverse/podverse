# Add-by-RSS Parity Subplan: Artists

## Goals
- Mirror non-add-by-RSS artist list and detail views under add-by-RSS.
...
## Scope
- Add-by-RSS artist list view.
- Add-by-RSS artist detail (channel) view.
...
## Steps
1. Define add-by-RSS artist list route and detail route mirroring non-add-by-RSS patterns.
   - Support paths: `/add-by-rss/artists`, `/add-by-rss/artist/:id`.
...
4. Ensure list/grid view behavior matches non-add-by-RSS artists list.
5. Import shared styles from `PodcastList.module.scss`.
6. Use local-only index values for route resolution.
7. Add feed URL title fallback for unparsed artist channels.

## Deliverables
- Add-by-RSS artist list/detail routes and components.
