...
- [x] All source files copied to `apps/api/src/`
- [x] `package.json` created with workspace dependencies
- [x] `tsconfig.json` extends base and has correct paths
- [x] `tsconfig.prod.json` created
- [x] `module-alias-config.ts` removed
- [x] `index.ts` updated to remove module-alias import
- [x] All imports updated to use `@podverse/*` packages
- [x] Express types (`@types/express.d.ts`) preserved
- [x] `npm run build -w apps/api` succeeds (0 errors, 104 warnings for non-null assertions)
- [x] `npm run lint -w apps/api` passes
- [ ] `npm run dev:api` starts (with valid .env) - **Not tested (requires env setup)**
...
4. `getManyByChannels` expects Channel[] but receives number[] (item.ts)
5. ItemChapter spread objects missing entity methods (item.ts)
6. `delete` operator on non-optional properties (account.ts, clip.ts)
