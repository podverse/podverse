...
- **Authoring:** Write migrations as **greenfield-only** ordered chains (assume prior files applied); avoid `IF EXISTS` / `IF NOT EXISTS` / seed `ON CONFLICT` / `WHERE NOT EXISTS` guards unless a predecessor in the same chain truly leaves ambiguity. See `.llm/exports/opencode/skills/linear-sql-greenfield-only/SKILL.md`.
