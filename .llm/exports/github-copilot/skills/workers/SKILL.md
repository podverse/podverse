---
name: workers
description: Per-job env validation and config patterns for the workers app. Use when adding or
  changing worker commands, touching workers startup validation, or documenting worker env vars.
version: 1.0.0
---
...
- **In-app imports**: use `@workers/*` for paths under `apps/workers/src` (full directory name). See `.llm/exports/github-copilot/instructions/app-internal-import-aliases.instructions.md`.
