...
- Use **table-sort-defaults** (`.llm/exports/github-copilot/skills/table-sort-defaults/SKILL.md`) for default **`asc`/`desc`** per column type.
- Use **sort-prefs-cookie-by-path** (`.llm/exports/github-copilot/skills/sort-prefs-cookie-by-path/SKILL.md`) when list metadata is persisted via cookies and URL stripping (see hooks in **`@podverse/ui`**).
