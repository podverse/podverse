...
- Secret generators in [infra/k8s/scripts/secret-generators/](scripts/secret-generators/INFRA-K8S-SCRIPTS-SECRET-GENERATORS.md) and other helpers in [infra/k8s/scripts/](scripts/README.md) (DB/Valkey connect, etc.) assume SOPS keys are available and `nix develop` provides required binaries.
