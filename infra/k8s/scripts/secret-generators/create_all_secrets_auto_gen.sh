...
MANUAL_SCRIPTS=(
	...
	"create_workers_storage_bucket_secret.sh (requires S3-compatible access/secret keys; see docs/image-shrinking/BUCKET-PROVIDERS.md)"
	"create_workers_garage_secret.sh         (requires Garage S3 access/secret keys; Secret podverse-workers-garage-opaque)"
	"create_github_registry_secret.sh        (requires GitHub username + PAT with read:packages for ghcr.io image pulls)"
	"create_mailer_secret.sh                (MAILER_USERNAME / MAILER_PASSWORD when ready)"
	"create_metaboost_secret.sh             (signing PEM + issuer when ready)"
)
...
OUTPUT_FILES=(
	...
	"./secrets/podverse-${ENVIRONMENT}-mailer-opaque.enc.yaml"
	"./secrets/podverse-${ENVIRONMENT}-metaboost-opaque.enc.yaml"
...
# -------------------------------------------------------------------
# Scripts to run when credentials are available (not part of --auto-gen pass for some)
# -------------------------------------------------------------------
echo ""
echo "Run the following when credentials are available:"
echo ""
