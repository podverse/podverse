...
# Encrypted files that may need optional stringData (sops edit) after --auto-gen; used for
# copy-paste commands at the end. Web push opaque has only the private VAPID key; see NOTE below.
OPTIONAL_MANUAL_SOPS_FILES=(
	"./secrets/podverse-${ENVIRONMENT}-api-opaque.enc.yaml"
)
...
echo ""
echo "NOTE: Some auto-generated secrets may still need optional values (sops edit) or env follow-up:"
echo "  - ./secrets/podverse-${ENVIRONMENT}-api-opaque.enc.yaml: MAILER_USERNAME and MAILER_PASSWORD are"
echo "    left empty by --auto-gen until you set them (only needed for outbound email)."
echo "  - Same file: METABOOST_SIGNING_KEY_PEM and METABOOST_APP_ASSERTION_ISS are empty until set;"
echo "    enable both together for App Assertion minting."
echo "  - Web push: the SOPS-encrypted file has only WEBPUSH_VAPID_PRIVATE_KEY. Public VAPID keys are"
echo "    autogenerated with the same pair and written to workers + web source env when paths exist."
echo "    WEBPUSH_VAPID_SUBJECT: set in infra/k8s/base/workers/source/workers.env and"
echo "    infra/k8s/base/api/source/api.env (monorepo), or apps/podverse-<env>/{workers,api}/source/*.env (GitOps)."
echo "    If the generator could not find env paths, it prints copy-paste public key lines at the end."
echo ""
echo "To edit or inspect optional API secret fields (mailer, Metaboost; copy-paste; requires SOPS key):"
echo ""
echo "  Open in \$EDITOR (edit stringData):"
for f in "${OPTIONAL_MANUAL_SOPS_FILES[@]}"; do
	echo "    sops ${f}"
done
echo ""
echo "  Decrypt to stdout (read-only check):"
for f in "${OPTIONAL_MANUAL_SOPS_FILES[@]}"; do
	echo "    sops -d ${f}"
done
echo ""
echo "  Apply one file to the cluster after you edit it (repeat per file as needed):"
for f in "${OPTIONAL_MANUAL_SOPS_FILES[@]}"; do
	echo "    sops -d ${f} | kubectl apply -f -"
done
