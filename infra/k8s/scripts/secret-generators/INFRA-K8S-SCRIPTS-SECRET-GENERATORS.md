...
- **`create_workers_add_by_rss_secret.sh` → `podverse-*-workers-add-by-rss-opaque.enc.yaml`**: always includes `ADD_BY_RSS_CREDENTIALS_ENCRYPTION_KEY`, the transit key API and workers share to seal and open Basic Auth credentials on the parse queue (nothing is encrypted at rest). Interactive mode allows an empty placeholder; the API still requires a valid 64-hex key before a healthy start if that env is required by your config.
...
- `create_grafana_admin_secret.sh` — Grafana UI admin for **cluster monitoring** (`grafana-admin` in namespace `grafana`, keys `admin-user` / `admin-password`). Default output **`./secrets/grafana-admin.enc.yaml`**. **Not** in `create_all_secrets_auto_gen.sh`. Prometheus/Grafana manifests live in your **GitOps** repo; apply this Secret before Argo syncs Grafana. Optional **`--output-file`**.
