...
| Visual primitives / polish    | `@podverse/ui` components + SCSS                                              | Shared RN primitives now; action affordances (Track 9c); **operator polish = Track 23**  | Tokens shared; components rebuilt; controls inventory shared |
...
| Playlists CRUD + resources    | `playlist/*` wrappers; `apps/web/src/app/playlist*`                           | Same wrappers; Track **9d** create/edit/reorder sketches; clip authoring deferred (21.4) | API full                                                     |
