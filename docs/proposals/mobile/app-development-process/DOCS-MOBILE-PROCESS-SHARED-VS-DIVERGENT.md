...
| Visual primitives / polish    | `@podverse/ui` components + SCSS                                              | Shared RN primitives now; **action affordances** (Play/more/subscribe) match web intents (Track 9c); pixel polish later | Tokens shared; components rebuilt; controls inventory shared |
