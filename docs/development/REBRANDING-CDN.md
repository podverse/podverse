...
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Light-theme UI wordmark | [`apps/web/public/branding/logo-rectangle.svg`](/apps/web/public/branding/logo-rectangle.svg) (`/branding/logo-rectangle.svg`)       | Dark artwork on light backgrounds; used when the UI theme is **light** ([`getBrandLogoSrc`](/apps/web/src/utils/brandLogo.ts)). Nav and footer render at **144×25** px ([`NavBarBrand`](/apps/web/src/components/NavBar/NavBarBrand.tsx), [`FooterBrand`](/packages/ui/src/components/layout/FooterLayout/FooterBrand.tsx) defaults). Canonical CDN filename: **`logo-rectangle.svg`**. Prefer **SVG**. |
| Dark-theme UI wordmark  | [`apps/web/public/branding/logo-rectangle-dark.svg`](/apps/web/public/branding/logo-rectangle-dark.svg) (`/branding/logo-rectangle-dark.svg`) | Light artwork on dark backgrounds; used for **dark**, **dracula**, and **violet** themes. Same **144×25** display size. Canonical CDN filename: **`logo-rectangle-dark.svg`**.                                                                                                                                                                        |
...
| Embed square brand mark | _(CDN only — not bundled in the image)_                                                                                                  | Canonical filename **`brand-square-100x100.png`** (**100×100** PNG). Host on your CDN and set **`NEXT_PUBLIC_BRAND_LOGO_SQUARE_100X100`** to the full URL; embed displays at **32×32** px. Omit the env var to hide the slot. See [Embed square brand mark](#embed-square-brand-mark-web-app-only-optional).                                                  |
...
If unset, the app uses bundled assets under `/branding/` in `public/` (defaults:
`logo-rectangle.svg` for light UI, `logo-rectangle-dark.svg` for dark UI).

| Convention           | Value                                                                                                                              |
| -------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| **Light filename**   | `logo-rectangle.svg`                                                                                                               |
| **Dark filename**    | `logo-rectangle-dark.svg`                                                                                                          |
| **CDN path example** | `https://<cdn>/static/images/branding/logo-rectangle.svg` (same `…/static/images/branding/` prefix as other brand assets)          |

**Migrating from `logo.svg` / `logo-dark.svg`:** Rename or re-upload CDN objects to the
`logo-rectangle*` filenames, update `BRAND_LOGO_LIGHT` / `BRAND_LOGO_DARK` URL values in home
`brand.env` (env **keys** stay the same), then run `make local_env_setup`. Deployments that still
point at old CDN filenames continue to work until operators update URLs.

---

## Embed square brand mark (web app only, optional)

The embed player can show a square mark in the upper-right info row (**32×32** px). It is **not**
the horizontal nav wordmark. There is no light/dark pair and no bundled fallback: when the embed
env var is empty, the embed omits the mark and its wrapper entirely.

| Variable                               | Brief role                                                                 |
| -------------------------------------- | -------------------------------------------------------------------------- |
| `NEXT_PUBLIC_BRAND_LOGO_SQUARE`        | Full **`https://`…** URL to the full-size square mark (reserved; unused) |
| `NEXT_PUBLIC_BRAND_LOGO_SQUARE_100X100` | Full **`https://`…** URL to the **100×100** embed mark                   |

| Convention           | Value                                                                                                                                                      |
| -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Full-size filename** | `brand-square.png` (reserved for future use)                                                                                                             |
| **Embed filename**   | `brand-square-100x100.png` (**100×100** PNG)                                                                                                             |
| **CDN path example** | `https://<cdn>/static/images/branding/brand-square-100x100.png` (same `…/static/images/branding/` prefix as header wordmarks)                              |
| **Embed display**    | **32×32** px (`object-fit: contain`)                                                                                                                       |

Local overrides: set `BRAND_LOGO_SQUARE` and/or `BRAND_LOGO_SQUARE_100X100` in
`dev/env-overrides/local/brand.env`; `local_env_setup` maps them to `NEXT_PUBLIC_BRAND_LOGO_SQUARE`
and `NEXT_PUBLIC_BRAND_LOGO_SQUARE_100X100` on the web sidecar. Only the **100×100** key drives the
embed.
