...
- Takedown handling uses a separate bulk-delete path.
...
- `infra/database/migrations/0001_init_podcasting_20_database.sql`
...
| --------------------------------------- | ------------------------------------------------------------- | -------------------------------------------------------------------- |
| Feed -> `Spam`                          | `packages/parser/src/lib/rss/parser.ts`                       | Parsed feed crosses spam limit (`>= 10000` items or live items).     |
...
| Feed -> any enum value (manual utility) | `apps/workers/src/commands/orm/feed/updateFlagStatus.ts`      | Explicit CLI command run with `podcast_index_id` and status id.      |
...
Parser only proceeds when feed status is `Active` or `AlwaysParse`.

- Gate: `checkIfFeedFlagStatusShouldParse(...)`
- Spam handling: parser sets feed status to `Spam` and aborts that run.
...
2. `processPendingArchiveItems()`
3. `removeAllItemsForTakedownFeeds()`
4. `pruneNonActiveItemBackedQueueResourceRows()` (delete `queue_resource` for non-`Active` items: direct `item_id`, and via `clip` / `item_soundbite` to a non-`Active` item; add-by-RSS rows are not deleted in this pass)
...
For regular archive processing:

- if item has `PlaylistResource` or `Clip`: set item status `Archived`,
- otherwise: delete item row.

After feed-level processing, feed is set `Archived` and `last_parsed_file_hash` is cleared.
...
| Only in queue                   | `queue_resource` is not a retention reason in `processItems`; pruned in step 4 / filtered on read | Hard delete path applies if no clip/playlist; any remaining non-`Active` `queue_resource` is removed in step 4. |
...
- Archiver includes TODO markers for spam-item handling and post-archive dependency rechecks.
