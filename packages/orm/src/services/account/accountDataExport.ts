...
export class AccountDataExportService {
  ...
  async exportUserData(account_id: number): Promise<{
    ...
    const followingAddByRSSChannels =
      await accountFollowingAddByRSSChannelService.getFollowedAddByRSSChannels(account_id);
    ...
    const playlistsData = await Promise.all(
      playlists.map(async (playlist) => {
        const resources = await playlistResourceService.getManyByPlaylistIdText(
          playlist.id_text,
          account_id,
          {
            relations: [
    ...
    const queuesData = await Promise.all(
      queues.map(async (queue) => {
        ...
        const allQueueResources = [...historyResources, ...upcomingResources].sort(
          (a, b) => parseFloat(a.list_position) - parseFloat(b.list_position)
        );
