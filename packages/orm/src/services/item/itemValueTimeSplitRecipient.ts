...
export class ItemValueTimeSplitRecipientService extends BaseManyService<
...
> {
  ...
  async update(
    ...
    dto: ItemValueTimeSplitRecipientDto
  ): Promise<ItemValueTimeSplitRecipient> {
    const whereKeys = [
      'type',
      'address',
      'custom_key',
      'custom_value',
    ] as (keyof ItemValueTimeSplitRecipient)[];
    return super._update(item_value_time_split, whereKeys, dto);
  }

  async updateMany(
    item_value_time_split: ItemValueTimeSplit,
    dtos: ItemValueTimeSplitRecipientDto[]
  ): Promise<ItemValueTimeSplitRecipient[]> {
    const whereKeys = [
      'type',
      'address',
