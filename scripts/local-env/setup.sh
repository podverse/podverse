...
upsert_var "$DB_ENV" "DB_APP_OWNER_PASSWORD" "$DB_APP_OWNER_PASSWORD"
upsert_var "$DB_ENV" "DB_APP_MIGRATOR_USER" "$DB_APP_MIGRATOR_USER"
upsert_var "$DB_ENV" "DB_APP_MIGRATOR_PASSWORD" "$DB_APP_MIGRATOR_PASSWORD"
...
upsert_var "$DB_ENV" "DB_MANAGEMENT_OWNER_USER" "$DB_MANAGEMENT_OWNER_USER"
upsert_var "$DB_ENV" "DB_MANAGEMENT_OWNER_PASSWORD" "$DB_MANAGEMENT_OWNER_PASSWORD"
upsert_var "$DB_ENV" "DB_MANAGEMENT_MIGRATOR_USER" "$DB_MANAGEMENT_MIGRATOR_USER"
upsert_var "$DB_ENV" "DB_MANAGEMENT_MIGRATOR_PASSWORD" "$DB_MANAGEMENT_MIGRATOR_PASSWORD"
...
# From rate-limit.env
for v in AUTH_LOGIN_MAX_PER_MINUTE ACCOUNT_CREATE_MAX_PER_10_MINUTES ACCOUNT_SEND_VERIFICATION_EMAIL_MAX_PER_10_MINUTES ACCOUNT_VERIFY_EMAIL_MAX_PER_10_MINUTES ACCOUNT_SEND_CHANGE_EMAIL_MAX_PER_10_MINUTES ACCOUNT_VERIFY_EMAIL_CHANGE_MAX_PER_10_MINUTES ACCOUNT_SEND_RESET_PASSWORD_EMAIL_MAX_PER_10_MINUTES ACCOUNT_RESET_PASSWORD_MAX_PER_10_MINUTES ACCOUNT_SET_PASSWORD_MAX_PER_10_MINUTES ACCOUNT_DOWNLOAD_DATA_MAX_PER_DAY ACCOUNT_OPML_EXPORT_MAX_PER_HOUR ACCOUNT_OPML_IMPORT_ENQUEUE_MAX_PER_HOUR ACCOUNT_ADD_BY_RSS_PARSE_ENQUEUE_MAX_PER_HOUR ACCOUNT_ADD_BY_RSS_CHAPTERS_TRANSCRIPT_MAX_PER_MINUTE ACCOUNT_CHANNEL_SEEN_READ_MAX_PER_MINUTE MQ_RSS_ON_DEMAND_MAX_PER_HOUR; do
	apply_override "$v" "${API_ENV_FILES[@]}"
done
# Shared OPML soft cap for API + workers (separate from HTTP enqueue 429 knobs).
apply_override "OPML_IMPORT_MAX_FEEDS_PER_HOUR" "${API_AND_WORKERS_ENV_FILES[@]}"
...
if [ -n "${CUSTOM_THEMES_URL:-}" ]; then
	for file in "${WEB_ENV_FILES_APP_AND_SIDECAR[@]}" "${MANAGEMENT_WEB_ENV_FILES_APP_AND_SIDECAR[@]}"; do
		upsert_var "$file" "NEXT_PUBLIC_CUSTOM_THEMES_URL" "$CUSTOM_THEMES_URL"
	done
fi
