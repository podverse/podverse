...
/**
 * Shared "GitHub-style" mirror: skills/, rules→instructions, root instructions file from .cursorrules.
 * Used by github-copilot and opencode export targets (portable tree under .llm/exports/<id>/).
 */
...
function rewriteCursorRefsToExportRoot(targetRoot, exportPathPosix) {
  ...
  for (const filePath of files) {
    ...
    const updated = original
      .replace(/\.cursor\/skills\//g, `${basePosix}/skills/`)
      .replace(
        /\.cursor\/rules\/([a-zA-Z0-9._-]+)\.mdc/g,
        (_m, p1) => `${basePosix}/instructions/${p1}.instructions.md`
      )
      .replace(/\.cursor\/rules\//g, `${basePosix}/instructions/`);
    if (updated !== original) {
 ...
 * @param {string} mainInstructionsFile - basename only, e.g. copilot-instructions.md
 */
function wipeGeneratedExport(targetRoot, mainInstructionsFile) {
  const skills = path.join(targetRoot, 'skills');
  const instructions = path.join(targetRoot, 'instructions');
  const main = path.join(targetRoot, mainInstructionsFile);
  if (fs.existsSync(skills)) {
    fs.rmSync(skills, { recursive: true, force: true });
  }
  if (fs.existsSync(instructions)) {
    fs.rmSync(instructions, { recursive: true, force: true });
  }
 ...
 * @param {string} opts.mainInstructionsFile - e.g. copilot-instructions.md, opencode-instructions.md
 * @param {string} opts.adapterLabel - for log line
 */
export function exportCursorToGithubStyleMirror(
  repoRoot,
  targetRoot,
  exportPathPosix,
  { full = false, mainInstructionsFile, adapterLabel }
) {
  if (full) {
    wipeGeneratedExport(targetRoot, mainInstructionsFile);
  }
  ...
  const instructionsDst = path.join(targetRoot, 'instructions');
  ...
  if (fs.existsSync(cursorRules)) {
    ...
    fs.writeFileSync(path.join(targetRoot, mainInstructionsFile), converted, 'utf8');
  }

  rewriteCursorRefsToExportRoot(targetRoot, exportPathPosix);

  console.warn(
    `  ${adapterLabel}: ${skillCount} skills, ${ruleCount} instruction files → ${path.relative(repoRoot, targetRoot)}`
  );
}
